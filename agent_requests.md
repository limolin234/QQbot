...
2. 匹配的写入today.jsonl(JSONL)文件中（记录 ts/group_id/user_id/user_name/chat_type/cleaned_message）
3. 随后匹配的消息会经过 bot.py 中的 urgent 和 normal关键词匹配
4. urgent_keywords匹配将作为urgent任务直接加入队列
5. normal_keywords匹配作为normal任务直接加入队列
6. 都不匹配的再做处理
7. 每天22：00 将自动将today.jsonl中的内容每10K字一组push到队列中处理(使用锁保护和临时文件替换，处理期间的新消息留给下一轮)
...
```mermaid
    ...
    CheckAllowed -->|是| WriteLog[写入today.jsonl(JSONL)]
    ...
    DailyCheck{是否22:00?}
    DailyCheck -->|是| BatchProcess[读取today.jsonl<br/>按10K字分组<br/>批量push到队列<br/>SUMMARY类型]
