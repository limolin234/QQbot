...
class DidaScheduler:
    ...
    async def handle_command(self, msg: GroupMessage | PrivateMessage) -> bool:
        ...
        if service is None:
            ...
            await self._send_text(
                msg,
                "⚠️ Dida 未配置，请开发者先在 agent_config.yaml 中填写 client_id/client_secret/redirect_uri。",
            )
            return True
        ...
        project_id = await self._ensure_default_project_id(
            user_id, tokens[user_id], service
        )
        if project_id:
            self.save_tokens(tokens)
        ...
        *,
        ...
        if project_id:
            ...
            projects = await asyncio.to_thread(
                service.get_projects, access_token=access_token
            )
            target_project_ids = []
            if isinstance(projects, list):
                target_project_ids = [
                    str(p.get("id") or "").strip() for p in projects if p.get("id")
                ]
        ...
        tasks_list = await asyncio.gather(
            *[
                asyncio.to_thread(
                    service.get_project_data, access_token=access_token, project_id=pid
                )
    ...
    async def execute_action_structured(
        ...
        *,
        ...
        if service is None:
            return {
                "ok": False,
                "message": "⚠️ Dida 未配置，请先在 agent_config.yaml 中填写 client_id/client_secret/redirect_uri。",
            }
        ...
        if target_user_id and target_user_id != user_key:
            ...
            if user_key in admin_qqs:
                ...
                return {
                    "ok": False,
                    "message": f"⚠️ 权限不足：你不是管理员，无法操作用户 {target_user_id} 的任务。",
                }
        ...
        if not isinstance(token_data, dict):
            return {
                "ok": False,
                "message": "⚠️ 尚未绑定 Dida 账号，请先发送 /dida_auth 获取授权链接。",
            }
        access_token = str(token_data.get("access_token") or "").strip()
        if not access_token:
            return {
                "ok": False,
                "message": "⚠️ Dida 授权信息无效，请重新绑定 /dida_auth。",
            }
        ...
        self._log(
            f"action_start type={action_type or 'unknown'} user={user_key} project={project_id or 'default'}"
        )

        if action_type == "create":
            if not project_id:
                project_id = await self._ensure_default_project_id(
                    user_key, token_data, service
                )
                if project_id:
                    self.save_tokens(tokens)
            ...
            if not project_id:
                return {
                    "ok": False,
                    "message": "⚠️ 未找到默认项目，请先在 Dida 中创建项目。",
                }
            ...
            if due_date_raw and not due_date:
                return {
                    "ok": False,
                    "message": "⚠️ due_date 格式不正确，请提供明确时间。",
                }
            ...
            if isinstance(reminders, list) and reminders:
                payload["reminders"] = [
                    str(item) for item in reminders if str(item).strip()
                ]
            created = await asyncio.to_thread(
                service.create_task, access_token=access_token, payload=payload
            )
            task_id = str(created.get("id") or created.get("taskId") or "").strip()
            self._log(
                f"task_created user={user_key} project={project_id} task={task_id or 'unknown'}"
            )
            ...
            if due_display:
                return {
                    "ok": True,
                    "message": f"✅ 已创建任务：{title} 📅 {due_display}",
        ...
        if action_type == "update":
            ...
            task_obj, pid = await self._find_task_obj(
                access_token, service, task_id=target_task_id, project_id=project_id
            )
            ...
            self._log(
                f"task_updated user={user_key} project={pid} task={payload['id']}"
            )
            ...
            if due_display:
                return {
                    "ok": True,
                    "message": f"✅ 已更新任务：{payload.get('title')} 📅 {due_display}",
                    "data": {"task_id": payload["id"]},
                }
            return {
                "ok": True,
                "message": f"✅ 已更新任务：{payload.get('title')}",
                "data": {"task_id": payload["id"]},
            }
        if action_type == "list":
            ...
            if not target_project_ids:
                return {
                    "ok": False,
                    "message": "⚠️ 未找到任何项目，请先在 Dida 中创建项目。",
                }
            ...
            if not total_tasks_count:
                return {
                    "ok": True,
                    "message": f"[CQ:at,qq={user_key}] 暂无未完成任务。",
                    "data": {"count": 0},
                }

            output_parts = [
                f"[CQ:at,qq={user_key}] 的任务清单 (共 {total_tasks_count} 项)"
            ]
        ...
        if action_type in {"delete", "complete"}:
            ...
            task_obj, pid = await self._find_task_obj(
                access_token, service, task_id=task_id, project_id=project_id
            )
            ...
            if action_type == "delete":
                ...
                self._log(
                    f"task_deleted user={user_key} project={target_project_id} task={target_task_id}"
                )
                return {
                    "ok": True,
                    "message": f"✅ 已删除任务：{task_obj.get('title', 'unknown')}",
                    "data": {"task_id": target_task_id},
                }
            ...
            self._log(
                f"task_completed user={user_key} project={target_project_id} task={target_task_id}"
            )
            return {
                "ok": True,
                "message": f"✅ 已完成任务：{task_obj.get('title', 'unknown')}",
                "data": {"task_id": target_task_id},
            }
    ...
    async def push_task_list_action(
        ...
        if not filtered_tasks:
            self._log(
                f"push_task_list: no tasks found for {target_user_qq} range={day_range}"
            )
        ...
        if group_id:
            await bot.api.post_group_msg(
                str(group_id), text=f"[CQ:at,qq={target_user_qq}]\n{message}"
            )
        else:
            await bot.api.post_private_msg(str(target_user_qq), text=message)

        self._log(
            f"push_task_list sent to {target_user_qq} (group={group_id}) count={len(filtered_tasks)}"
        )
    ...
    async def poll_once(self, config: dict[str, Any]) -> None:
        ...
        tokens = self.load_tokens()
        self._log(
            f"poll_once users={len(tokens)} projects_config={len(config.get('project_ids', []))}"
        )
        for user_id, token_data in tokens.items():
            ...
            if not project_ids:
                try:
                    ...
                    if isinstance(projects, list):
                        ...
                        self._log(
                            f"poll_projects_fetched user={user_id} projects={project_summary}"
                        )
                        project_ids = [
            ...
            for project_id in project_ids:
                ...
                if project_id == "inbox" and project_name == "unknown":
                    project_name = "Inbox"
                self._log(
                    f"poll_project user={user_id} project={project_name}({project_id}) tasks={len(tasks)}"
                )
                ...
                if not tasks and isinstance(data, dict):
                    ...
                    self._log(
                        f"poll_project_empty_detail keys={list(data.keys())} project_meta={project_info}"
                    )
                ...
                for task in tasks:
                    ...
                    if bool(task.get("isAllDay")):
                        due_display = (
                            due_dt.astimezone().strftime("%Y-%m-%d") + " (全天)"
                        )
                    message = f"⏰ 任务到期提醒\n标题：{title}\n到期：{due_display}\nID: {task.get('id')}"
    ...
    async def _send_route(
        self, chat_type: str, target_id: str, mention_user_id: str, text: str
    ) -> None:
        if chat_type == "group":
