...
直接运行：clone下来 配置好 .env ./workflows/agent_config.yaml 直接跑main.py ncatbot库会自动引导配置config.yaml并安装配置napcat
...
docker-compose容器化安装：
  ...
  windows:  
    手动去release里下载压缩包解压，配置文件要自己填所以脚本也就是自动下载解压  
  
  注：napcat官方有docker脚本 需自行安装  

## Config Studio 可视化配置面板 (前端界面)

本项目提供了一个配套的 Web 可视化配置界面 (Config Studio)，方便用户在浏览器中管理 Agent 配置和定时任务 (Scheduler)。
...
### 2. 设计背景与核心概念：Agent vs Scheduler

在 Config Studio 中，我们将机器人的配置能力严格剥离为 **Agent 界面** 和 **Scheduler 界面**，两者各司其职：

#### 🔴 Agent 界面：负责“做什么”(What) 和“怎么做”(How)
*   **身份设定与思考方式**：使用什么模型、温度是多少、人设 Prompt 是什么。
*   **服务范围与规则**：对哪些群生效（生效列表/黑名单）、匹配什么关键词才触发、历史上下文该记住几条。


#### 🟢 Scheduler 界面：负责“什么时候做”(When)
*   **设计目的与背景**：随着不同 Agent 的增多，需要一个集中式的调度中心来管理自动化任务，解耦业务逻辑与触发时机。
*   **定时触发**：每天晚上 22:30 定时（Cron表达式）。
*   **循环轮询**：每隔 10 分钟拉取一次滴答清单（Interval间隔）。
*   **Scheduler 界面就是一个高级的“闹钟”或者“事件触发器”。对于**像总结 Agent (`summary.daily_report`) 和 滴答 Agent (`dida.poll`) 等复杂代理，它们都是靠自身的 Rules 配置运行的，Scheduler 只需要负责唤醒它们即可。** 这一个纯粹的“唤醒”动作就足够了。

### 3. 实现的效果
通过这种解耦设计，实现了以下优势：
- **互不干扰**：修改机器人的回复人设和规则（修改 Agent），完全不影响每天晚上的定时任务；反之亦然。
- **动态调整定时计划**：在 Web 端通过可视化界面修改 Cron 表达式，增删定时任务等，并可预览时间线，甚至热更新推送到远端服务器并自动重启，全程无需改动后端代码。
- **一站式配置体验**：前端还提供了直观的 `.env` 密钥管理和项目快照恢复功能，将配置可视化和核心引擎运行完美结合。

### 4. 推送中心：一键云端部署与更新
*   **设计目的**：传统的修改配置往往需要手动 SSH 登录服务器、替换文件和打补丁，过程繁琐且易错。推送中心的设计初衷是实现一套**“所见即所得”的可视化部署流程**，让用户在本地浏览器上完成设定的调整后，能够快速同步到云端。
*   **实现效果**：通过 SFTP 协议自动打包本地经过修改的配置（如 `agent_config.yaml` 和 `.env`等），一键推送到远端正在运行 QQ 机器人的服务器，并在云端自动执行重启指令（如重启 Docker 容器），令新的人设、新配置或定时任务即刻生效，实现了零门槛的远程机器人配置热更新。
